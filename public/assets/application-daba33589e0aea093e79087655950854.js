...
(function() {
  jQuery(function() {
    var landing_zone_map, order_submission;
    $("#new_order").submit(function(event) {
      event.preventDefault();
      if (navigator.geolocation) {
        return navigator.geolocation.getCurrentPosition((function(position) {
          $('#lz-button').addClass('disabled');
          return order_submission(position);
        }), function() {
          return alert('something went wrong getting your location.  Please try reloading the page and resubmitting your request');
        }, {
          enableHighAccuracy: true
        });
      } else {
        return alert('your browser does not support HTML5 Geolocation!');
      }
    });
    $('#thank-you').hide();
    landing_zone_map = function(position) {
      var handler;
    ...
    return order_submission = function(position) {
      return $.ajax({
        type: "POST",
